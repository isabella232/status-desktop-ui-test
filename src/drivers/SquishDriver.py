...
# *****************************************************************************/
# Generic view components definitions:
# *****************************************************************************/
# It defines high level Login View components:
class StatusLoginViewComponents(Enum):
	MAIN_VIEW 					= names.loginView_main
	PASSWORD_INPUT 				= names.loginView_passwordInput
	SUBMIT_BTN 					= names.loginView_submitBtn
	CHANGE_ACCOUNT_BTN			= names.loginView_changeAccountBtn
	ERR_MSG_LABEL 				= names.loginView_errMsgLabel

# It defines high level Account View components:
class StatusAccountsPopupComponents(Enum):
	ACCOUNTS_POPUP				= names.accountsView_accountListPanel
...
def is_loaded_visible_and_enabled(objName, timeout = _MAX_WAIT_OBJ_TIMEOUT):
	obj = None
	try:
		obj = squish.waitForObject(objName, timeout)
		return True, obj
	except LookupError:
		return False, obj
